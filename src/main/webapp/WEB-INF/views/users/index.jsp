...
<div id="idTransformView" style="height: 425px; overflow: hidden; position: relative;width: 1196px;">
		...
		<c:forEach items="${frontList}" var="cate">
			<div class="xm-box">
				<div class="title">
					...
					<h2>${cate.catename }</h2>
					<a class="more" href="cate.action?id=${cate.cateid }">更多 </a>
				</div>
				<div id="show_new_area" class="clearfix xm-boxs">
					<!-- 循环开始  -->
					<c:forEach items="${cate.goodsList}" var="goods">
						<div class="goodsItem goodsItems">
							<a href="detail.action?id=${goods.goodsid }"><img src="<%=path %>/showImg.action?imgurl=${goods.image}" alt="${goods.goodsname }"
																			  class="goodsimg" /> </a> <br />
							<p class="f1">
								<a href="detail.action?id=${goods.goodsid }" title="${goods.goodsname }">${goods.goodsname }</a>
							</p>
							<p class="">
